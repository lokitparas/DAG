...
if __name__ == '__main__':
    ...
    for quality in threshold:
        ...
        for i in range(len(_data)):
            ...
            #print(img_labels)
            img_labels = torch.tensor(np.array([img_labels]),dtype=torch.int64)
            print(img_labels)
            ...
            ori_img_ = torch.tensor(ori_img_).unsqueeze(0)
            ori_img_ = Variable(ori_img_.cuda(),requires_grad=True)
            ...
            print(ori_img_.size())
            _, _, _, _, _, image_iteration=DAG(model=faster_rcnn,
                  image=ori_img_,
